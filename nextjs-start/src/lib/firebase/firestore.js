...
function convertToDate(firestoreDate) {
  ...
  export async function getEventsSnapshot(user, cb) {
    ...
    // Update the query to filter events based on RBAC
    const q = query(collection(db, "events"), where("rbac", "array-contains", user.uid));
    ...
    const unsubscribe = onSnapshot(q, querySnapshot => {
        const events = querySnapshot.docs.map(doc => {
            ...
            return {
                id: doc.id,
                ...data,
                // Adjusted to show date strings directly
